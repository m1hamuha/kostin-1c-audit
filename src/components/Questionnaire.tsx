  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-8 md:px-8 md:py-12">
      ...
      <div key={i} className="animate-rise">
        ...
        <div className="mt-6">
          {step.kind === "single" && (
            <SingleField step={step} answers={a} set={setA} onPick={() => !isLast && setTimeout(next, 160)} />
          )}
          {step.kind === "multi" && <MultiField step={step} answers={a} set={setA} />}
          {step.kind === "pains" && <PainsField step={step} answers={a} set={setA} />}
        </div>
      </div>
...
function stepComplete(step: Step, a: Answers): boolean {
  ...
  if (step.kind === "multi") return a[step.key].length > 0;
  // pains: optional — allow proceeding even with nothing selected
  return true;
}
